class Gamelist::PaymentNotification < Gamelist::Db
  ...
  def payment_amount_correct()
    ...
    (1..params[:num_cart_items].to_i).each do |i|
      product = Gamelist::Product.find(params["item_number#{i}"])
  ...
  def save_order
    customer = Gamelist::Customer.where(:payer_id => params[:payer_id])
    customer_id = customer[0].id if !customer.empty?
    if customer.empty?
      customer = Gamelist::Customer.new(
    ...
    order = Gamelist::Order.where(:gamelist_payment_notification_id => id)
    if order.empty?
      order = Gamelist::Order.new(
        :gamelist_payment_notification_id => id,
        :gamelist_customer_id => customer_id,
